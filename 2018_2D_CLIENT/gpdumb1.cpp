...
int Draw_BOB32(BOB_PTR bob)             // bob to draw
{
	...
	if (bob->effect_time > GetTickCount() - 250)
		Draw_Effect(bob);
...
int Draw_Effect(BOB_PTR target) 
{
	...
	// is bob visible
	if (!(target->attr & BOB_ATTR_VISIBLE))
		return(1);

	D3DXVECTOR3 pos[4] = { D3DXVECTOR3((target->x - g_left_x - 1) * static_cast<float>(TILE_WIDTH) + 10,
		(target->y - g_top_y) * static_cast<float>(TILE_WIDTH) + 10, 0.0)
		, D3DXVECTOR3((target->x - g_left_x + 1) * static_cast<float>(TILE_WIDTH) + 10,
		(target->y - g_top_y) * static_cast<float>(TILE_WIDTH) + 10, 0.0)
		, D3DXVECTOR3((target->x - g_left_x) * static_cast<float>(TILE_WIDTH) + 10,
		(target->y - g_top_y - 1) * static_cast<float>(TILE_WIDTH) + 10, 0.0)
		, D3DXVECTOR3((target->x - g_left_x) * static_cast<float>(TILE_WIDTH) + 10,
		(target->y - g_top_y + 1) * static_cast<float>(TILE_WIDTH) + 10, 0.0) };

	RECT my = { effect.tx[effect.curr_frame], effect.ty[effect.curr_frame], // Å©±â
		effect.tx[effect.curr_frame] + effect.width, effect.ty[effect.curr_frame] + effect.height };

	for(int i = 0; i < 4; ++i)
		g_pSprite->Draw(effect.images[effect.curr_frame], &my, NULL, &pos[i],D3DCOLOR_ARGB(255, 255, 255, 255));
